...
//	}
	    	...
	    	for (int y = 0 ; y != _dy ; y++ ) {
				...
				if(Buffer0[x][y].getType()<0) {//si la case contient de l'eau
					...
					for (int z = 0; z<voisins.size();z++) {
						...
						if (voisins.get(z).getType()<0 && voisins.get(z).getHauteur()>Buffer0[x][y].getHauteur()){//si la case n'a pas d'eau et est plus basse que la case oÃ¹ il y a de l'eau
	...
	public void InitIsland(ImageBuffer hm, ImageBuffer mm,ImageBuffer tm) {
		...
		for ( int x = 0 ; x != _dx ; x++ ) {
			for ( int y = 0 ; y != _dy ; y++ ) //on parcourt les 3 images donnÃ©es et on remplit la matrice de cases en fonction
			{
